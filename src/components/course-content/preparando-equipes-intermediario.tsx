...
export default function PreparandoEquipesIntermediarioContent({ course, isPurchased }: { course: Course; isPurchased: boolean }) {
  ...
  const courseData = tObject<any[]>('courses.preparando-equipes-intermediario.data') || [];
  
  return (
    <div className="space-y-4">
        <div className="text-left mb-8 prose prose-sm prose-invert max-w-none text-foreground space-y-6">
            <h2 className="text-2xl font-bold text-primary font-headline">{t('courses.preparando-equipes-intermediario.header.title')}</h2>
            
            <div>
                <h3 className="font-bold text-lg text-primary">{t('courses.preparando-equipes-intermediario.intro.welcome.title')}</h3>
                <p><strong>Dante:</strong> "{t('courses.preparando-equipes-intermediario.intro.welcome.dante')}"</p>
                <p><strong>Djeny:</strong> "{t('courses.preparando-equipes-intermediario.intro.welcome.djeny')}"</p>
            </div>

            <div>
                <h3 className="font-bold text-lg text-primary">{t('courses.preparando-equipes-intermediario.intro.method.title')}</h3>
                <p><strong>Dante:</strong> "{t('courses.preparando-equipes-intermediario.intro.method.dante')}"</p>
                <p><strong>Djeny:</strong> "{t('courses.preparando-equipes-intermediario.intro.method.djeny')}"</p>
            </div>
            
            <div>
                <h3 className="font-bold text-lg text-primary">{t('courses.preparando-equipes-intermediario.intro.pro.title')}</h3>
                <p><strong>Dante:</strong> "{t('courses.preparando-equipes-intermediario.intro.pro.dante')}"</p>
                <p><strong>Djeny:</strong> "{t('courses.preparando-equipes-intermediario.intro.pro.djeny')}"</p>
            </div>

            <div>
                <h3 className="font-bold text-lg text-primary">{t('courses.preparando-equipes-intermediario.intro.cta.title')}</h3>
                <p><strong>Dante:</strong> "{t('courses.preparando-equipes-intermediario.intro.cta.dante')}"</p>
                <p><strong>Djeny:</strong> "{t('courses.preparando-equipes-intermediario.intro.cta.djeny')}"</p>
                <p><strong>Dante & Djeny:</strong> "{t('courses.preparando-equipes-intermediario.intro.cta.final')}"</p>
            </div>
        </div>
      <Accordion type="single" collapsible className="w-full">
        {courseData.map((item: any, index: number) => {
            const stage = activeStage[index];
            const danteAudioId = `dante-preparando-${index}`;
            const djenyAudioId = `djeny-preparando-${index}`;
                    ...
                    <div className="space-y-4">
                        {!stage && (
                            <div className="rounded-lg border-2 border-primary/20 bg-zinc-950/60 backdrop-blur-md p-6 shadow-xl shadow-black/40 mb-8">
                                <p className="text-muted-foreground mb-4">{t('courses.preparando-equipes-intermediario.modules.stage.dante')}</p>
      ...
      <Card className="mt-8 text-center bg-gradient-to-br from-primary/10 to-secondary/10 border-primary/30 shadow-lg">
          <CardHeader>
              <CardTitle className="font-headline text-2xl text-primary flex items-center justify-center gap-2">
                  <Award className="h-6 w-6"/> {t('courses.preparando-equipes-intermediario.closing.title')}
              </CardTitle>
          </CardHeader>
          <CardContent className="prose prose-sm prose-invert max-w-none text-muted-foreground space-y-4">
              <p><strong>Dante:</strong> "{t('courses.preparando-equipes-intermediario.closing.dante1')}"</p>
              <p><strong>Djeny:</strong> "{t('courses.preparando-equipes-intermediario.closing.djeny1')}"</p>
              <p><strong>Dante:</strong> "{t('courses.preparando-equipes-intermediario.closing.dante2')}"</p>
              <p><strong>Djeny:</strong> "{t('courses.preparando-equipes-intermediario.closing.djeny2')}"</p>
              <p className="font-bold text-lg text-primary">Dante & Djeny: "{t('courses.preparando-equipes-intermediario.closing.final')}"</p>
          </CardContent>
      </Card>
      
      <Card className="mt-6 text-center text-sm border-2 border-primary/20 bg-zinc-950/60 backdrop-blur-md p-4 shadow-xl shadow-black/40">
         <CardHeader className="p-2">
            <CardTitle className="flex items-center justify-center gap-2 text-primary font-headline tracking-widest">
                <Zap className="h-5 w-5" />
                {t('courses.preparando-equipes-intermediario.mentoria.title')}
            </CardTitle>
         </CardHeader>
         <CardContent className="p-2 text-muted-foreground">
            <p className="font-bold text-foreground">{t('courses.preparando-equipes-intermediario.mentoria.bonus')}</p>
            <p className="mt-2">{t('courses.preparando-equipes-intermediario.mentoria.attention')}</p>
            <ul className="mt-2 list-disc list-inside text-left space-y-1">
                <li><span className="font-semibold text-primary">{t('courses.preparando-equipes-intermediario.mentoria.item1.title')}:</span> {t('courses.preparando-equipes-intermediario.mentoria.item1.desc')}</li>
                <li><span className="font-semibold text-primary">{t('courses.preparando-equipes-intermediario.mentoria.item2.title')}:</span> {t('courses.preparando-equipes-intermediario.mentoria.item2.desc')}</li>
            </ul>
         </CardContent>
      </Card>
    </div>
