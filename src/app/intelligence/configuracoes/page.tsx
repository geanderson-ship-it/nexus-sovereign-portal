...
import { CheckCircle, Shield, Gem, Users, Building, Zap, UserCircle, Loader2 } from 'lucide-react';
...
export default function ConfiguracoesPage() {
  ...
  const [nickname, setNickname] = useState(user?.name || user?.displayName || '');
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      await updateUserAttributes({
        userAttributes: {
          name: nickname.trim(),
          given_name: nickname.trim(),
        }
      });
      toast({
        title: 'Perfil atualizado!',
        description: `Agora você será chamado de ${nickname.trim()}.`,
      });
    } catch (error: any) {
      toast({
        variant: 'destructive',
  ...
  return (
    <div className="space-y-8">
      ...
      <Card className="bg-zinc-950/60 border-2 border-primary/20 backdrop-blur-md shadow-xl shadow-black/40">
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="font-headline text-xl">Meu Perfil</CardTitle>
              <CardDescription>Como você quer ser chamado na plataforma?</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                id="nickname"
                placeholder="Ex: Gean, Comandante, Geanderson..."
                value={nickname}
                onChange={(e) => setNickname(e.target.value)}
                className="bg-black/30 border-zinc-700"
              />
              <p className="text-xs text-muted-foreground">Este nome aparecerá no Bem-vindo e nas IAs.</p>
            </div>
            <div className="space-y-2">
              <Label>E-mail</Label>
              <Input value={user?.email || ''} disabled className="bg-black/30 border-zinc-700 opacity-50" />
            </div>
          </div>
        </CardContent>
        <CardFooter>
          <Button onClick={handleSaveProfile} disabled={isSaving} className="bg-primary hover:bg-primary/90">
            {isSaving ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Salvando...</> : 'Salvar Perfil'}
          </Button>
        </CardFooter>
      </Card>
      ...
      <div className="max-w-7xl mx-auto py-12">
        <LegalSafeguard module="NEXUS CONFIG" protocol="NX-CONF-01" />
      </div>
    </div>
  );
}
