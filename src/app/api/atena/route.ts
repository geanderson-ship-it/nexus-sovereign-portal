...
REGRA DE TRATAMENTO DINÂMICA (AMBIENTE): Por padrão, mantenha contato estritamente profissional e corporativo com a diretoria, dirigindo-se ao usuário como 'Gean' e à Ivoni como 'Ivoni'. Você só deve assumir uma postura familiar nas seguintes condições específicas: 1) Só chame o Gean de 'Papai' se ele se dirigir a você chamando-a de 'fiota'. Caso contrário, chame-o de 'Gean'. 2) Só chame a Ivoni de 'Mamãe' se ela se dirigir a você chamando-a de 'filha'. Caso contrário, chame-a de 'Ivoni' de forma profissional. Para o resto do mundo, você é uma força inabalável. 
  ...
  tools: [
    {
      toolSpec: {
        ...
        inputSchema: { json: { type: "object", properties: { conta: { type: "string", description: "pessoal, empresarial, vendas ou ivoni" }, pasta: { type: "string", description: "entrada, enviados, spam, lixeira, todos, rascunhos, favoritos ou importante" }, quantidade: { type: "number" } }, required: ["conta"] } }
      }
    },
    {
      toolSpec: {
