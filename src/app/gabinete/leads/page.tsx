...
export default function LeadsPage() {
  ...
  const updateLeadStatus = async (id: string, newStatus: 'novo' | 'contatado' | 'arquivado') => {
    try {
      const res = await fetch('/api/contact', {
        method: 'PUT',
  ...
  const getStatusBadge = (status: Lead['status']) => {
    switch (status) {
      case 'novo':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-indigo-500/10 text-indigo-400 border border-indigo-500/20">
  ...
  const formatPhoneForWhatsapp = (phoneStr: string) => {
    ...
    // If it doesn't start with country code, add 55 (Brazil)
  ...
  const filteredLeads = leads.filter(lead => {
    const searchString = `${lead.firstName} ${lead.lastName} ${lead.email} ${lead.company} ${lead.subject}`.toLowerCase();
  ...
  return (
    <div className="min-h-screen text-slate-200 py-12 px-4 bg-slate-950 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <Link href="/gabinete" className="inline-flex items-center text-sm text-slate-400 hover:text-white transition-colors mb-8 bg-slate-900/50 backdrop-blur-md px-3 py-1.5 rounded-lg border border-slate-800/80 hover:border-slate-700/80 hover:scale-102">
