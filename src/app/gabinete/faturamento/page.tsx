...
import { Lock, DollarSign, TrendingUp, TrendingDown, Clock, ChevronLeft, Download, Building2, Calendar } from 'lucide-react';
...
const MOCK_PAGOS = [
  ...
  { id: 'FAT-2026-84', cliente: 'Hospital Moinhos', modulo: 'Mensalidade Health', data: '05/06/2026', valor: 'R$ 19.900', status: 'Liquidado' },
];
...
const MOCK_A_RECEBER = [
  ...
  { id: 'FAT-2026-93', cliente: 'TechCorp Solutions', modulo: 'Nexus ERP Mensal', vencimento: '15/06/2026 (Atrasado)', valor: 'R$ 15.000', risco: 'Alto' },
];
...
export default function FaturamentoPage() {
  ...
  const [activeTab, setActiveTab] = useState<'pagos' | 'areceber'>('pagos');
    ...
    <div className="min-h-screen text-slate-200 pt-32 pb-20 px-4 relative">
      ...
      <div className="relative z-10 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          <Card className="bg-slate-900/40 border-emerald-900/50 shadow-[0_0_15px_rgba(16,185,129,0.05)]">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-3xl font-headline text-emerald-400">R$ 1.579.900</CardTitle>
            </CardHeader>
          </Card>
          ...
          <Card className="bg-slate-900/40 border-amber-900/50">
            <CardHeader className="pb-2">
              <CardDescription className="text-xs uppercase tracking-widest font-bold text-amber-500 flex items-center gap-2">
                <Clock className="w-3 h-3" /> A Receber (Próx. 15 dias)
              </CardDescription>
              <CardTitle className="text-3xl font-headline text-amber-400">R$ 469.499</CardTitle>
            </CardHeader>
          </Card>

          <Card className="bg-slate-900/40 border-rose-900/50">
            <CardHeader className="pb-2">
              ...
              <CardTitle className="text-3xl font-headline text-rose-400">R$ 15.000</CardTitle>
            </CardHeader>
          </Card>

          <Card className="bg-slate-900/40 border-blue-900/50">
            <CardHeader className="pb-2">
              <CardDescription className="text-xs uppercase tracking-widest font-bold text-blue-500 flex items-center gap-2">
                <Building2 className="w-3 h-3" /> Ticket Médio B2B
              </CardDescription>
              <CardTitle className="text-3xl font-headline text-blue-400">R$ 213.600</CardTitle>
            </CardHeader>
          </Card>
        </div>
        ...
        <div className="flex gap-4 mb-6 border-b border-slate-800 pb-px">
          <button 
            ...
            className={`pb-3 text-sm font-bold uppercase tracking-widest transition-colors relative ${activeTab === 'pagos' ? 'text-emerald-400' : 'text-slate-500 hover:text-slate-300'}`}
          >
            Faturas Liquidadas (Pagas)
            {activeTab === 'pagos' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]" />}
          </button>
          <button 
            onClick={() => setActiveTab('areceber')}
            className={`pb-3 text-sm font-bold uppercase tracking-widest transition-colors relative ${activeTab === 'areceber' ? 'text-amber-400' : 'text-slate-500 hover:text-slate-300'}`}
          >
            Contas a Receber
            {activeTab === 'areceber' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-amber-500 shadow-[0_0_10px_rgba(245,158,11,0.5)]" />}
          </button>
        </div>
                      ...
                      <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded text-xs border ${
                            fatura.vencimento.includes('Atrasado') 
