...
const batch2Lectures = [
    { slug: 'motivacao-e-engajamento', title: 'Engajamento Real: Propósito Além do Salário.' },
    { slug: 'inteligencia-emocional', title: 'Blindagem Emocional e Maturidade Profissional.' },
    { slug: 'seguranca-psicologica', title: 'Confiança Radical: Onde a Inovação Acontece.' },
    { slug: 'cultura-de-alta-performance', title: 'DNA de Elite: Construindo uma Cultura de Resultado.' },
    { slug: 'gestao-de-conflitos', title: 'Do Conflito ao Acordo: Negociação Interna.' }
];
...
async function generateChunk(slug: string, title: string, startIdx: number, endIdx: number, previousContext: string) {
    ...
    Você DEVE retornar APENAS um JSON válido contendo as chaves necessárias. Exemplo do formato:
    {
      ...
      "lectures.scripts.${slug}.item${startIdx+1}.text": "Texto do bloco ${startIdx+1}...",
    ...
    while (retries > 0) {
        try {
            ...
            for(let i = startIdx; i <= endIdx; i++) {
                if(!generatedData[`lectures.scripts.${slug}.item${i}.text`]) {
                    console.log(`Missing key item${i}.text! Retrying...`);
                    allKeysPresent = false;
                    break;
                }
            }
            if(!allKeysPresent) throw new Error("Missing keys in chunk");

            return generatedData;
            
        } catch (e) {
            retries--;
            console.error(`❌ Failed to generate chunk [${startIdx}-${endIdx}], retries left: ${retries}`, e.message);
...
async function runLectures() {
    for (const lecture of batch2Lectures) {
